...
public:
	Channel();
	Channel(const std::string name, const unsigned int& sid, const std::vector<unsigned int>& pids);

	friend bool YAML::convert<Channel>::decode(const YAML::Node& node, Channel& rhs);
...
inline bool YAML::convert<Channel>::decode(const YAML::Node& node, Channel& c) {
	...
	for(const_iterator it = node["pids"].begin(); it != node["pids"].end(); ++it)
	{
