...
ServerRunner::ServerRunner(const std::string &confFile,
  ...
  broadcastListGeneratorPtr_ =
      std::make_shared<BroadcastListGenerator>(configPtr_);
}
...
void ServerRunner::startCommandDispatcher() { commandDispatcherPtr_->start(); }

void ServerRunner::stopCommandDispatcher() { commandDispatcherPtr_->stop(); }
...
void ServerRunner::startWatchdog() { watchdogPtr_->start(); }

void ServerRunner::stopWatchdog() { watchdogPtr_->stop(); }
